...
{
	if (DamageType != none && DamageType != Class'Zedternal.WMUpgrade_Napalm_DT' && DamageInstigator != none && MyKFPM != none && FRand() < (float(DefaultDamage) * default.maxProbability / default.maxDamage))
	{
		//add fire/microwave effects on zed
		MyKFPM.ApplyDamageOverTime(int(float(DefaultDamage)*default.damageFactor[upgLevel-1]), DamageInstigator, Class'Zedternal.WMUpgrade_Napalm_DT');
	}
}
...
defaultproperties
{
	...
	upgradeIcon(0)=Texture2D'Zedternal_Resource.Skills.UI_Skill_Napalm'
	upgradeIcon(1)=Texture2D'Zedternal_Resource.Skills.UI_Skill_Napalm_Deluxe'
}